...
import { outboxEventProducerUseCase } from '@use-cases/outbox-event-producer';

const tracer = new Tracer();
const metrics = new Metrics();

// we pull the events off the queue and publish the messages to the events topic
export const processStreamAdapter = async ({
  ...
  try {
    ...
    const recordPromises = Records.map(async (record) => {
      ...
      return await outboxEventProducerUseCase(record);
    });
    ...
    metrics.addMetric('EventProcessorError', MetricUnit.Count, 1);
